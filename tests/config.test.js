...
test("readConfig defaults to all supported providers", () => {
  assert.deepEqual(defaultConfig().providers, ["codex", "claude", "glm", "deepseek", "minimax", "kimi", "qwen"]);
});
...
test("ensureConfig creates default config once", () => {
  ...
  assert.deepEqual(readConfig(paths).providers, ["codex", "claude", "glm", "deepseek", "minimax", "kimi", "qwen"]);
});
